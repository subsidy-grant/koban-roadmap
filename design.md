...
### 全数監査・第2弾（2026-08-13(5)、本人指示「全数監査して」）

サイト全体128制度を機械的に洗い出し、名称に「補助」「助成」どちらも含まない
制度が、前回確定させた6件のほかに**さらに5件**あると判明した。shindanshiが
公式サイトの一次情報で判定し、`PROGRAM_TYPE_OVERRIDE`に追加した：

| キー | 判定 | 根拠 |
|---|---|---|
| chiba_chiba_ict（千葉市ICT活用等生産性向上支援事業） | 補助金 | 正式名称は「助成金」だが、企業支援会議による非公開の競争的審査・予算上限で早期終了・1年度1企業1回制限があり実質は補助金 |
| kanagawa_ebina_shinko（海老名市中小企業振興支援事業） | 補助金 | 10メニュー全て公式に「補助金」用語、先着順・予算上限あり |
| kanagawa_yamato_koten（大和市魅力ある個店支援事業） | 補助金 | 「補助金」用語、プレゼンテーション審査会による明確な競争選考 |
| chiba_sodegaura_shinko（袖ケ浦市企業振興条例奨励金） | 助成金 | 「奨励金」用語、固定資産税納付相当額連動の機械的算定。**条例施行規則本文（PDF）は未到達、予算上限条項の有無は未確認の留保あり** |
| saitama_sayama_ritchi（狭山市企業立地奨励金等交付制度） | 助成金 | 「奨励金」用語、固定資産税相当額連動、客観要件を満たせば対象、競争選考の記載なし。**要綱全文までは未到達の留保あり** |

**残課題**：名称に「補助」「助成」の片方だけを含む制度が117件残っており、
これらは今回の全数監査の対象外（本人の指示で次回以降に持ち越し）。江東区
ICT等導入支援事業（「補助金」の名称だが要件充足型で実質は助成金的）のような
名実不一致は、117件のどこかにさらに潜んでいる可能性がある。
