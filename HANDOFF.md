...
## 次の安全なアクション（最新・2026-08-15第12セッション、documents.html 7項目改修まで完了・push済み）

**本セッションで行った作業はすべてコミット・push済み（最終コミット`050396a`、2026-08-15時点でorigin/mainと同期確認済み）。次に着手すべきは、本人が予告した「複数会社情報を所持しているときの自動入力バグチェック」。**

次の安全なアクションは、**複数会社データでの自動入力の網羅的バグチェックに着手すること**。観点は本人指示のとおり「会社1と会社2の情報が正確に反映されるか、混同しないか」「誤入力・行のずれが発生しないか」。本セッション中に1制度（career_shoyo）×2社では.docx実ファイルレベルでデータ分離を確認済みだが、これは氷山の一角（対象は数百〜千件規模の様式）。次のセッションでは複数制度・複数様式形式（.docx/.xlsx）・様式内の複雑な書き分け（法人/個人事業主の欄分岐、住所の分割欄等）を横断した検証が必要。着手前に規模を見積もり、本人に時短案（サンプリング方針、並列verify等）を提示してから始めること（本人指示の恒久ルール、[[feedback_propose_time_savers]]）。

再開者が仮定してはいけないこと：
- 「documents.htmlは01セクション（会社情報入力）を持つ」と仮定しないこと。第12セッションで完全に`profile.html`へ分離済み。documents.htmlは02（制度選択・書類入力、`#autofill`ひとつだけ）のみで構成される
- 「会社情報は1社分（`koban_company`）」と仮定しないこと。第12セッションで複数会社対応（`koban_companies`配列、`company_data.js`の`window.KOBAN_COMPANY` API）に変更済み。旧データは自動で「会社1」に移行される
- 「documents.htmlにTOC・セクション見出し・アコーディオン開閉がある」と仮定しないこと。本セッションで全て撤去し、ページを開くと即座に「手順1」から見える単純な構成になった
- 「様式のバッジは常に緑の『自動入力可』」と仮定しないこと。`mirrorOf(url)`が偽の様式（このサイトに複製が無い、CORSで取得失敗しうる）は赤系「取り寄せて記入を試みます（サイトによっては失敗することがあります）」に変わっている
- 「下部タブバーは4タブでホームが左端」と仮定しないこと。第12セッションで5タブ（🔍探す／📄書類準備／✅申請進捗／📅スケジュール／🏢会社情報）に拡張、高さも56px→68pxに拡大。会社情報タブは意図的に右端配置（本人指示「使う頻度が少ないから」）
- 「サイトはアプリ化されていない（PWA機能なし）」と仮定しないこと。第12セッションの前半で`manifest.json`・`sw.js`（Service Worker、現在キャッシュ名`koban-roadmap-v3`）を導入済み。ホーム画面に追加してアプリのように使える
- 前回（第11セッション）の引き継ぎに書かれた「03削除→02統合」「TOC整理」等は**本セッションで別の形（会社情報の完全独立化）に発展的に解消済み**。第11セッションの引き継ぎ本文は下に残すが、現状と矛盾する記述は本セッションの内容を優先すること
