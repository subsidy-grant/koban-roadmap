# -*- coding: utf-8 -*-
"""【2026-08-16に廃止】このスクリプトはもう実行しないこと。

業種別・改善計画10選は独立ページ improvement.html に分離され（build_hub.py が生成）、
本体 index.html 側のマーカー(IMPROVEMENT_HUB_EMBED)は撤去済み。実行するとマーカーも
挿入先のCTA段落も見つからず SystemExit で異常終了する（意図した安全弁）。
このファイル自体は経緯の記録として残しているだけで、削除はしていない。
