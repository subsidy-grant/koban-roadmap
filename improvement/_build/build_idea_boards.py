...
<body>
<p class="back" style="max-width:1200px;margin:14px auto 0;padding:0 24px;font-size:14px;"><a href="../index.html" style="color:{cfg["accent"]};font-weight:700;text-decoration:none;">← 改善計画10選へ戻る</a> ／ <a href="../../index.html#improvement-cta" style="color:{cfg["accent"]};font-weight:700;text-decoration:none;">ロードマップ本体へ</a></p>
<header>
  <h1>{label}×AI システム 1000案 — PDCA評価ダッシュボード</h1>
  <p>20カテゴリ × 10コア機能 × 5業態({cfg["segline"]})。5軸スコア(市場性・省力化度・実現性・補助金適合・差別化)でPlan→Do→Check→Actの絞り込みを実施。<b>TOP10は「改善計画10選」として採択し、それぞれにA4×10ページの事業計画書（Excel版つき）を用意しています</b>。選定は100施策スコアリング(improvement/_build/data/scores/)の上位領域({cfg["stage1_areas"]})と突き合わせています。なお試作プロトタイプは美容業・飲食業のみの公開で、{label}は未整備です。</p>
</header>
<div class="wrap">
  ...
  <p class="topnote">この10案が{label}の<b>改善計画10選</b>です。各案にA4×10ページの事業計画書（印刷・PDF保存可／Excel版つき）があります。数値は公開情報・業界目安にもとづくモデルケースの試算で、実申請時は自社の実績データに差し替えてご利用ください。試作プロトタイプは{label}では未整備です。</p>
...
<footer>koban-roadmap {label}×AI 1000案 / スコアは決定的シードによる机上評価(実データでの再評価を推奨)。TOP10は本ボードの5軸評価による採択案で、各案に事業計画書（モデルケース）を用意しています。100施策の機械スコアリングの結果は improvement/_build/data/scores/ に記録している。</footer>
