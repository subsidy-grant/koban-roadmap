...
// ---- カラーパレット ----
const C = { ink:"#26221e", sub:"#6f675e", line:"#d9d2c8", accent:"#8a5a2b", green:"#3e6b4f", gold:"#b98a2f", red:"#a4453c", bg:"#faf8f5" };
...
{ no:1, key:"01-ai-multilingual-blog", proto:"01-ai-multilingual-blog.html", scheme:"業務改善",
  ...
  saveH:30, saveY:129, roi:1.4, wageUp:60,
...
{ no:4, key:"04-ai-meo-review", proto:"04-ai-meo-review.html", scheme:"業務改善",
  ...
  saveH:12, saveY:96, roi:1.6, wageUp:60,
...
{ no:5, key:"05-ai-short-video", proto:"05-ai-short-video.html", scheme:"業務改善",
  ...
  saveH:15, saveY:108, roi:1.5, wageUp:60,
...
{ no:8, key:"08-ai-training-video", proto:"08-ai-training-video.html", scheme:"業務改善",
  ...
  saveH:24, saveY:130, roi:1.6, wageUp:60,
...
{ no:10, key:"10-ai-repeat-follow", proto:"10-ai-repeat-follow.html", scheme:"業務改善",
  ...
  saveH:12, saveY:110, roi:1.4, wageUp:60,
...
function schemeInfo(scheme, p) {
  if (scheme === "省力化") {
    const rate = "1/2(小規模事業者 2/3)";
    const subAmt = Math.round(p.inv * 0.5);
    return {
      badge: "中小企業省力化投資補助金(一般型)", color: C.green,
      rate, subAmt, self: p.inv - subAmt,
      cap: "従業員規模に応じ最大1億円(大幅賃上げ特例で上限引上げ)",
      wage: "【大幅賃上げ特例(任意)】事業場内最低賃金+3.0%/年 かつ 給与総額+3.5%/年 を計画期間で達成すると補助率2/3・上限引上げ",
      audit: "省力化(労働生産性の向上)を定量的に示せるか。付加価値額の年平均成長率など事業計画の妥当性・実現可能性。",
    };
  }
  const before = 1180, up = p.wageUp, rate = before < 1050 ? "4/5" : "3/4";
  const subAmt = Math.min(600, Math.round(p.inv * 0.75));
  return {
    badge: "業務改善助成金", color: C.gold,
    rate: "3/4(引上げ前の事業場内最低賃金が1,050円未満は4/5)", subAmt, self: p.inv - subAmt,
    cap: "最大600万円(引上げ額・人数に応じたコース別上限)",
    wage: `【必須要件】事業場内最低賃金を${up}円以上引き上げ(本計画: ${before}円→${before+up}円)。生産性向上に資する設備投資等が対象。`,
...
function buildPlan(p) {
    ...
    <div class="split">
      <div class="half">
        ...
        <table class="data">
          ...
          <tr><td>事業場内最低賃金</td><td>1,180円</td><td><b>${1180 + (p.scheme === "業務改善" ? p.wageUp : 40)}円</b></td></tr>
          <tr><td>引上げ額</td><td>—</td><td>+${p.scheme === "業務改善" ? p.wageUp : 40}円</td></tr>
  ...
  const P10 = `
    ...
    <table class="data wide">
      ...
      <tr><td>${p.scheme === "業務改善" ? "事業場内最低賃金の引上げ" : "賃上げ計画(特例活用時)"}</td><td>+${p.scheme === "業務改善" ? p.wageUp : 40}円の引上げを計画</td><td class="ok">✔</td></tr>
...
function wageChart(p) {
  const up = p.scheme === "業務改善" ? p.wageUp : 40;
  return `<svg viewBox="0 0 300 170" width="100%">
    ...
    ${[["現行",1180,90],["計画1年",1180+up,150],["計画3年",1180+up+30,210]].map(([l,v,x],i)=>{
      const h = (v-1100)*1.1;
...
const exportData = PLANS.map(p => {
  ...
  cf.push({ y: "導入時", cum: Math.round(cum) });
  for (let y = 1; y <= 4; y++) { cum += netY; cf.push({ y: `${y}年目`, cum: Math.round(cum) }); }
  const wageUpApplied = p.scheme === "業務改善" ? p.wageUp : 40;
  return {
    ...p,
    scheme_info: s,
    opex, netY, cf,
    wage_before: 1180, wage_up_applied: wageUpApplied,
    wage_after1: 1180 + wageUpApplied, wage_after3: 1180 + wageUpApplied + 30,
    growth_pct: 3 + p.no % 3,
  };
});
